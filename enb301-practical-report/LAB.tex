...
The reason the frequency has no effect on these values, as explained in section~\ref{sec:d}, is that the circuit contains no energy storing devices (such as capacitors or inductors), and as such- frequency should not affect any circuit properties. Additionally, the approximated transfer function of this system does not has no elements that depend on the input frequency, or more simply;
