...
Part B considered an open loop servo motor system and used the prelab completed in part A to calculate the $K_m$ and $\alpha$ values. 

Part C and D considered the close loop response and a detailed examination of closed loop aspects were investigated. A PID controller was used to optimise the system.\\
Time response characteristics were also analysed.
...
\begin{enumerate}
	\item A longer and more thorough introduction to some of the more technical features matlab has to offer.
	\item Reduce the complexity of the practical layout.
	\item Introduce assessed weekly lab sessions devoted to experiment preparation. 
\end{enumerate}
...
\label{sec:d}
\begin{enumerate}
	\item If time permitted, a bodeplot would be included.
...
function [ te_new,ye_new, ye_stepNew ] = timing_fix_D3(te,ye,ye_step)
	...
	\item \textbf{Build closed loop model of servo motor and replicate the closed loop response obtained in Part C step 6.}
	\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{Matlab_Code/Figures/D4_Simulink.eps}
	\caption{\label{fig:rand}Simulated PID Controller}
	\end{figure}

	\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{Matlab_Code/Figures/D4_experimental.eps}
	\caption{\label{fig:rand}Closed loop response Experimental}
	\end{figure}

	\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{Matlab_Code/Figures/D4_SimulinkModel.png}
	\caption{\label{fig:rand}Simulated PID Controller Simulink}
	\end{figure}

\pagebreak
	\item \textbf{Propose a PID controller that achieves a faster response. Simulate close-loop response and plot on same graph as the closed loop reponse of the Part C step 6 controller}

	\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{Matlab_Code/Figures/D5.eps}
	\caption{\label{fig:rand}PID Controller Vs Closed Loop Response}
	\end{figure}

	\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{Matlab_Code/Figures/D5_Simulink.png}
	\caption{\label{fig:rand}PID Controller and Closed Loop Response Simulink}
	\end{figure}
