...
\begin{enumerate}
	...
	\end{figure}
    \begin{align*}
	\frac{V_{m}(s)}{V_{o}(s)} &= \frac{R_2}{R_1}
	\end{align*}\\
...
\textbf{System Transfer Function:}\\
	\begin{align*}
	G_{c}(s) &= \frac{V_p(s)}{V_i(s)}\\ 
	&= \frac{V_i(s)}{V_d(s)}\times\frac{V_d(s)}{V_{in}(s))}\\
	&= \frac{K_m}{s(s+\alpha)}\times\frac{R_2}{R_1}\times(V_i(s)-V_p(s))\\
	&=\frac{-K_m R_2}{R_1}\bigg(\frac{Vi(s)-V_p(s)}{s(s+\alpha)}\bigg)
	\end{align*}
...
From Difference Op Amp: $V_o(s) = V_i(s) - V_p(s)$
	...
	&= \frac{\frac{K_m R_2}{R_1}}{s^2+\alpha R_1 K_m R_2s+\frac{K_m R_2}{R_1}}
	\end{align*}	  
