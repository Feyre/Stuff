...
After measuring and recording the systems response, overshoot and settling time, for five different input frequencies; the following table was constructed. 

\begin{center}
    \begin{tabular}{| c | c | c |}
    \hline
    Input Frequency (Hz)  & Overshoot (\%)    & Settling time (s) 	\\ \hline
    0.5  	                  & 6.1                   & 1 			\\ \hline
	0.75  	                  & 5.7                   & -1 			\\ \hline
	1		                  & 6.1                   & 1			\\ \hline
	1.25	                  & 6.5                   & 1			\\ \hline
...
\begin{enumerate}
	...
	\item The overshoot and settling time were measured over five different input frequencies in order to determine the effect over varying the input signals frequency. The systems response to these signals was also recorded, and can be found as a set of graphical time domain plots below.
	\begin{figure}[H]
	  \begin{subfigure}{0.5\textwidth}
	  \includegraphics[width=0.9\linewidth]{Matlab_Figures/y2_dataset1_ms.eps} 
	...
	\pagebreak
	The peak overshoot and \textbf{settling time} were measured using the cursors \textbf{stuuff}\\
...
\end{center}
	...
	Eventually, as the frequency increases, the output doesn't have enough time to settle. 	It is also evident from the previous figures, that neither the overshoot, settling time or the steady state error is effected; until eventually the system does not have enough time to reach a steady state value.\\
	Freq only affects time available to find a steady state value. \\
	Eventually, there op-amps physical components would not be able to switch fast enough, and \textbf{WHAT} would occur. \\
	
	
	
	This occurs because \textbf{why}\\
	Frequency doesn't affect zeta and omega, so settling time and OS should stay the same.\\
	no energy storing components in the circuit.
	...
	As you would expect, changing the gain value \textbf{increases OS?} and \textbf{decreases Ts?}. The above images show large evidence to support this comment; as the correlation between these values can be seen clearly. \\
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Closed loop response figures (theoretical gain or given gain?)
		\item What can be seen from the figures
		\item Experimental gain figures
	...
	\begin{enumerate}
		...
		\item Check tables settling time
		\item Find word
		\item Check results are right (freq doesnt affect system)
		\item Why is the effect of gain useful?
		\item \textbf{SIMULINK MODEL}
		\item \textbf{PI/PD controller}
	\end{enumerate}
	\item \textbf{DISCUSSION and RECOMMENDATIONS}
	\item Answers (B) - add more words?
	\item Answers (C)
	\begin{enumerate}
		\item Fix figures
		\item Add non-ideal method
		\item C(4)
		\item Finish C(7)
		\item \textbf{NEW METHOD for finding TF}
	\end{enumerate}
	...
	\begin{enumerate}
		\item \textbf{DO NEW METHOD}
		\item Does frequency affect anything?
		\item Prove how gain affects shit
		\item \textbf{SIMULINK MODEL}
		\item \textbf{PI/PD controller}
	\end{enumerate}
\end{enumerate}
